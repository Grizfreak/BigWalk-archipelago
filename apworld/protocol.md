...
| `start_with_arch_doors_open` | list[str] | The option as chosen (item names of the doors open from the start), for Universal Tracker. The mod reads the list below instead. Replaced `lock_arch_doors` (str: `all`, `far`, `far_left`, `far_right`, `disabled`), which 0.1.1 seeds still carry. |
