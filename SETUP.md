...
replicate like any other game state, and the vanilla props removed for the
filler items are removed for everybody.

Two things are not. They are local display decisions the game never sends
over the network, so a player without the mod simply does not get them:

- the purple postgame gourds revealed on the map — they stay hidden on theirs;
